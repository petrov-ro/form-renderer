    ...
    return (
        <div className={`statistics-grid-container component-${id}`}>
            {value
                .map(({
                          id: rowId,
                          content: cols = [] as StatisticsGridCol[]
                      }) =>
                    <Row gutter={[5, 5]} style={{paddingBottom: 5}} key={rowId}>
                        {cols
                            .map(({
                                      id: colId,
                                      content: colElements = [] as StatisticsFormElementClass[]
                                  }) =>
                                <Col span={colSpan} key={colId}>
                                    <DropTarget onDrop={onDrop(rowId, colId, colElements)}
                                                className='statistics-grid-col'>
